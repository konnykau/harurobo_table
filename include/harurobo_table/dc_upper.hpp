...
class DC_upper_vel{
    ...
    const uint16_t CAN_ID;//CAN ID
    upper_state Upper_state;
    const float VEL_TARGET;

    public:
    DC_upper_vel(int CAN_ID,float TARGET)//velocityモードで使う用
    :CAN_ID(CAN_ID),Upper_state(upper_state::down),VEL_TARGET(TARGET)
    {}//初期化
    std::unique_ptr<can_plugins2::msg::Frame> update(bool rise_condition,bool fall_condition){
        if(rise_condition){
            return rise_by_vel();
        }
        else if(fall_condition){
            return fall_by_vel();
        }
        else{
            return stop_vel();
